(define (domain assembly)
   (:requirements :adl :equality :existential-preconditions)
   ...
   (:action commit
      ...
      :precondition (and (available ?res) (requires ?as ?res))
   ...
   (:action assemble
      ...
      :effect (and (incorporated ?part ?whole)
           (not (available ?part))
           (when (and (part-of ?part ?whole)
                              (not (exists (?p - assembly)
                      (and (part-of ?p ?whole)
                                       (not (= ?p ?part))
   ...
   ; end.
   (:action remove
      :parameters (?part ?whole - assembly ?res - resource)
      :precondition (and (forall (?res - resource)
                (imply (requires ?whole ?res)
                   (committed ?res ?whole)))
      ...
      :effect (and (not (incorporated ?part ?whole))
           (available ?part)
           (when (and (transient-part ?part ?whole)
                              (not (exists (?p - assembly)
                      (and (part-of ?p ?whole)
                       (not (incorporated ?p ?whole)))))
                  (not (exists (?tp - assembly)
