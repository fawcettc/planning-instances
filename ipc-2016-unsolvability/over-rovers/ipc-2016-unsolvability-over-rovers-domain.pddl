...
(:action navigate
:parameters (?x - rover ?y - waypoint ?z - waypoint ?energypost - energylevel ?energydelta - energylevel ?energypre - energylevel)
:precondition (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y)
                (visible ?y ?z) (energycost ?energydelta ?y ?z) (energy ?x ?energypre) (sum ?energypost ?energydelta ?energypre))
:effect (and (not (energy ?x ?energypre)) (energy ?x ?energypost) (not (at ?x ?y)) (at ?x ?z)
        )
)
(:action sample_soil
